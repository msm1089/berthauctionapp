...
const validateVesselInput = require('../../validation/vessel');

router.get(
  '/',
  passport.authenticate('jwt', { session: false }),
  (req, res) => {
    Vessel.find({ operator: req.user.user_name })
      .then((vessels) => res.status(200).json(vessels))
      .catch((err) =>
        res
          .status(400)
          .json({ user: 'Error fetching vessels of logged in user' })
      );
  }
);
